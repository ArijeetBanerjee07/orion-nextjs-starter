    ...
    const init = async () => {
      ...
      const run = (timestamp: number) => {
        ...
        if (isClicking) {
          ...
          targetIntensity = scrollIntensity;
        } else {
      ...
      const handleScroll = () => {
        if (!enableScrollFuzz) return;
        const currentScrollY = window.scrollY;
        const delta = Math.abs(currentScrollY - lastScrollY);
        lastScrollY = currentScrollY;
        
        isScrolling = true;
        scrollIntensity = Math.min(baseIntensity + (delta * 0.02), hoverIntensity * 1.5);
